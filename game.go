	...
	for {
		...
		for {
			...
			if bytes.HasPrefix(line, []byte("data: ")) {
				纯数字 := bytes.TrimSpace(line[6:])
				当前对方世代, err := strconv.ParseInt(B2S(纯数字), 10, 64)
				if err != nil {
					continue
				}
...
func 执行优雅重启(生命周期 context.Context, 提示消息 string, 自定义等待时间 *uint32) {
	...
	if 洞穴是否开启 {
		发送纯文本指令(`if #GetPlayerClientTable() == 0 then c_shutdown(true) end`, 发往洞穴)
	}
	...
	if 主世界是否开启 {
		...
		发送纯文本指令(`c_save(); print("K_SAVED")`, 发往洞穴)
	}
...
func 实时输出流(节点类型 uint8, 前缀 string, 读取器 io.Reader, 就绪标记 *atomic.Bool) {
	...
	for {
		...
		if 节点类型 == 节点_主世界 {
			...
			if 位置 != -1 {
				前缀部分 := 行字节[:位置]
				if bytes.Contains(前缀部分, 特征_玩家聊天前缀) {
